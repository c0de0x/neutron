...
public:
    ...
    void Unserialize(Stream& s, int nType, int nVersionDummy)
    {
        ...
        if (nLost + nLostUnk > 0) {
            LogPrintf("addrman lost %i new and %i tried addresses due to collisions\n", nLostUnk, nLost);
        }
    ...
    void Attempt(const CService &addr, bool fCountFailure = 1, int64_t nTime = GetAdjustedTime())
    {
        LOCK(cs);
        Check();
